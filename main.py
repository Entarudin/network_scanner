...
def main():
    ip_address = get_ip_shell_executor.execute()
    list_network_interface = get_network_interfaces_executor.execute().split('\n')
    network_interface = get_network_interface_by_ip(list_network_interface, ip_address)
    ...
    network_address = cast_ip_to_network_address(ip_address)
    ...
    protocols = xml_service.to_dict(REPORT_SCAN_PROTOCOLS_XML)

    json_repository.write_to_json(OUTPUT_RESULT_FILE, ip_address, hostname, networks_interfaces, network_interface,
                                  dict_info_distribution_version,
                                  info_core, list_hosts, list_services_status, list_installed_packages,
